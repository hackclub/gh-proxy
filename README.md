...
| `DB_MAX_CONNS`               | No                            | `20`                                                                                                                                                             | Max Postgres connections per instance. Keep instances × this × 2 (rolling deploys) under the server's `max_connections` minus reserved slots. |
| `DB_MAX_IDLE_CONNS`          | No                            | `2`                                                                                                                                                              | Minimum connections the pool keeps open (pgxpool `MinConns`).                                                                        |
| `DB_CONN_MAX_LIFETIME`       | No                            | `1800`                                                                                                                                                           | Seconds before a connection is recycled (with 10% jitter).                                                                           |
